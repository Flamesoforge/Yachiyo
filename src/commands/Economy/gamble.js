...
export default {
    ...
    execute: withErrorHandling(async (interaction, config, client) => {
        ...
        if (cloverCount > 0) {
            ...
            cloverMessage = `\n🍀 **Lucky Charm Used (${charmCount - 1} left):** Your win chance was boosted!`;
            usedCharm = true;
        }
        ...
        // 4. Corrected Financial Math Logic
        if (win) {
            const totalPayout = Math.floor(betAmount * PAYOUT_MULTIPLIER);
            cashChange = totalPayout - betAmount; // Net profit added to wallet

            resultEmbed = successEmbed(
                "🎉 You Won!",
                `You successfully gambled and turned your **$${betAmount.toLocaleString()}** bet into **$${totalPayout.toLocaleString()}**!${cloverMessage}`,
            );
        } else {
            cashChange = -betAmount; // Exact loss subtracted from wallet
        ...
        // Apply state changes to the data object
